        ...
        loop
            ...
            if validate=0 then --SI NO LO ESTA, INSERTO Y SALGO DEL CICLO
                ...
                if (array(random_promo) = '2x1') then
                    ...
                    (seq_promocion.nextval, array(random_promo), 50, monto((precio_vr*(50*0.01)),unidad), fecha(vr_fecha,vr_fecha+5), vr_id_plato);
                else if (array(random_promo) = '3x2') then
                    insert into promocion values 
                    (seq_promocion.nextval, array(random_promo), 30, monto((precio_vr*(70*0.01)),unidad), fecha(vr_fecha,vr_fecha+5), vr_id_plato);
                else
                    insert into promocion values 
                    (seq_promocion.nextval, concat(array(random_promo),'%'), to_number(array(random_promo)), monto((precio_vr*((100 - to_number(array(random_promo)))*0.01)),unidad), fecha(vr_fecha,vr_fecha+5), vr_id_plato);
                end if;
                end if;
